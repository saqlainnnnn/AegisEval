...
from app.metrics.retrieval.precision_at_k import PrecisionAtKMetric


def test_precision_metric_returns_one() -> None:
    dataset = Dataset(
    ...
    metric = PrecisionAtKMetric(k=5).compute(evaluation)

    assert metric.metric == MetricType.PRECISION_AT_K
    assert metric.value == 1.0


def test_precision_metric_respects_k() -> None:
    dataset = Dataset(
        metadata=DatasetMetadata(
            name="Test",
            description="Test",
    ...
    metric = PrecisionAtKMetric(k=2).compute(evaluation)
